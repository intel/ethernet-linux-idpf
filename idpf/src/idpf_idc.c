...
idpf_idc_init_msix_data(struct idpf_adapter *adapter)
{
	...
	privd = (struct iidc_rdma_priv_dev_info *)cdev_info->iidc_priv;
			       ...
			       enum iidc_function_type ftype)
{
	...
	privd->mapped_mem_regions = kcalloc(adapter->hw.num_lan_regs,
	...
	for (i = 0; i < adapter->hw.num_lan_regs; i++) {
		privd->mapped_mem_regions[i].region_addr =
			(u8 __iomem *)adapter->hw.lan_regs[i].vaddr;
