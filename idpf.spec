...
%if 0%{?sign_operation_name:1}%{?sign_certificate_name:1} != 0
echo "Sign %{?sign_file_name} using %{?sign_operation_name} and certificate %{?sign_certificate_name}"
idpf_ko_file=$(find . -name %{?sign_file_name})
chmod +x %{?signing_script}
%{?signing_script} $idpf_ko_file %{?sign_operation_name} %{?sign_certificate_name} sha512
%endif
